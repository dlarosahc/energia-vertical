...
const selectScheduleSection = document.querySelector('#select-schedule-section');
...
//Filtro por Calendario para agregarse en la clase que uno quiera
inputDate.addEventListener('change', async e => {
  selectScheduleSection.innerHTML = ''; 
  const selectedDate = inputDate.value;
  const objectDate = new Date(selectedDate + 'T00:00:00');
  console.log(objectDate);
  const dayIndex2 = objectDate.getDay();
  let filteredDay = daysOfWeek[dayIndex2];
  console.log(filteredDay);
  scheduleSection.classList.remove('flex');
  scheduleSection.classList.add('hidden');
  selectScheduleSection.classList.remove('hidden');
  selectScheduleSection.classList.add('flex');
  ...
  try {
    const { data } = await axios.get('/api/schedule', {
        ...
        <div class="max-w-3xl mx-auto text-center">
            ...
            <div class="mt-4">
             <p id="day-date" class="inline-flex items-center text-lg font-medium text-primary-600">${objectDate.toISOString().split('T')[0]}</p>
            </div>
          </div>
        `;

        selectScheduleSection.append(dayHead);

   if (filteredDay === 'Lunes') {
    monday.forEach(schedule => {
     ...
     `;
    selectScheduleSection.append(scheduleDiv);
      
        
        
    })
    
   } else if (filteredDay === 'Martes') {
...
selectScheduleSection.addEventListener('click', async e => {
  //Aqui se  agrega el alumno a la clase
  if(e.target.closest('.schedule-btn')){
 ...
 // Aqui se ven los alumnos asignados a la clase
  if(e.target.closest('.see-btn')){
    ...
    scheduleAttendance.children[0].children[0].innerHTML = title.innerHTML;
    scheduleAttendance.children[0].children[1].innerHTML = time.innerHTML;
    scheduleAttendance.children[0].children[2].value = inputDate.value;
    const closeBtn = scheduleAttendance.children[2];
    ...
    const selectedSchedule = data.filter(({ schedule, date }) => schedule === scheduleId && date === inputDate.value  );
    console.log(selectedSchedule);
    console.log(dateNow);
